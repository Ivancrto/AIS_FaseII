...
public class pruebasUnitariasDobles{
	...
	public void generico() throws ShopException  {
		//Given
			...
			//When
				if(pedidoGenerico.get(0).getIdC()==3) {
					ProductStockWithdrawExceededException e1= Assertions.assertThrows(ProductStockWithdrawExceededException.class, ()-> purchaseServ.createPurchase(3, pedidoGenerico.get(0).getIdP()));
					//Preguntar al profesor si hay que hacer un assertEquals
				}
				else if (pedidoGenerico.get(0).getIdC()==2) {
					CustomerCreditLimitExceededException e2= Assertions.assertThrows(CustomerCreditLimitExceededException.class, ()-> purchaseServ.createPurchase(2, pedidoGenerico.get(0).getIdP()));
					//Preguntar al profesor si hay que hacer un assertEquals
				}
				else {
